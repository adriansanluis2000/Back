...
describe("PedidoController", () => {
  ...
  describe("actualizarPedido", () => {
    ...
    it("debería devolver 200 y mensaje si el stock se devuelve con éxito", async () => {
      req.params.id = "1";
      const resultado = { productosActualizados: [/* lo que sea */] };
      pedidoService.devolverStock.mockResolvedValue(resultado);

      await pedidoController.devolverStock(req, res);

      expect(pedidoService.devolverStock).toHaveBeenCalledWith("1");
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
        mensaje: "Stock devuelto con éxito",
        resultado,
      });
    });

    it("debería manejar errores al devolver el stock", async () => {
      req.params.id = "1";
      const error = new Error("Error al devolver stock");
      pedidoService.devolverStock.mockRejectedValue(error);
